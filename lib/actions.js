...
Actions.prototype.deploy = function(option) {
  var self = this;
  self._showKadiraLink();

  //var buildLocation = path.resolve(os.tmpdir(), uuid.v4());
  var now = new Date();
  var datePath = now.toJSON().split('T')[0];
  var buildLocation = "/tmp/mup/" + this.config.appName  + '/' + datePath; 
  ...
  if (option == "nobuild") {
    ...
    console.log('Upload and deploy only'.blue); // + this.config.app);
    ...
    async.mapSeries(
      ...
      whenAfterDeployed(buildLocation, option)
    );

  } else  {

    console.log('Building ...'); // + this.config.app);

    // Clean up
    rimraf.sync(buildLocation);

    buildApp(appPath, meteorBinary, buildLocation, this.config, function(err) {
      if(err) {
        process.exit(1);
      } else {
        console.log('Build and Archive complete'); // + this.config.app);
        var sessionsData = [];
        _.forEach(self.sessionsMap, function (sessionsInfo) {
          var taskListsBuilder = sessionsInfo.taskListsBuilder;
          _.forEach(sessionsInfo.sessions, function (session) {
...
function whenAfterDeployed(buildLocation, option) {
  console.log("")
  return function(error, summaryMaps) {
    if (option == "save") {
      console.log("Build saved in " + buildLocation.green);
    } else if (option == "nobuild") {
      console.log("Build still in " + buildLocation.green);
    } else {
      console.log("Cleaning up temp build directory".green);
      rimraf.sync(buildLocation);
    }
